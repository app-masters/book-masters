  ...
  try {
    ...
    const authors = await Promise.all(
      bookData.authors.map(async author => {
        return (await axios.get(`https://openlibrary.org${author.key}.json`)).data.name;
      })
    );

    const works = (await axios.get(`https://openlibrary.org${bookData.works[0].key}.json`)).data;
    const bookResp = {};
    ...
    bookResp.publishingCompany = bookData.publishers ? bookData.publishers.join(',') : '';
    bookResp.imageUrl = `http://covers.openlibrary.org/b/isbn/${isbn}.jpg`;
    bookResp.publicationYear = bookData.publish_date ? new Date(bookData.publish_date).getFullYear() : '';
